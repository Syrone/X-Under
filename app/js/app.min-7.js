...
document.addEventListener('DOMContentLoaded', function () {
  //COOKIE
  var versionBtns = document.querySelectorAll('.version-button');
  var welcomePage = document.querySelector('.welcome');
  var currentPath = window.location.pathname;
  var malePath = currentPath.endsWith('main-male');
  var femalePath = currentPath.endsWith('main-female');
  if (versionBtns) {
    versionBtns.forEach(function (button) {
      button.addEventListener('click', function () {
        var date = new Date();
        date.setTime(date.getTime() + 365 * 24 * 60 * 60 * 1000);
        document.cookie = "version=".concat(button.getAttribute('data-version'), "; path=/; expires=").concat(date.toUTCString());
        location.reload();
      });
    });
  }
  ...
  if (malePath || femalePath) {
    ...
    links.forEach(function (link) {
      link.addEventListener('click', function (e) {
        ...
        var url = new URL(currentUrl.origin + currentUrl.pathname);
  ...
  window.addEventListener('popstate', function () {
    ...
    var newVersion = urlParams.get('version');
    if (newVersion) {
      version = newVersion;
      if (version === 'female') {
        document.body.classList.add('female');
  ...
  function getCookie(name) {
    var nameWithEquals = "".concat(name, "=");
  ...
  if (document.getElementById('hearXUnderModal')) {
    var formSubmittedKey = 'formSubmitted';
    var showModal = function showModal() {
      var hearXUnderModal = new bootstrap.Modal(document.getElementById('hearXUnderModal'));
      hearXUnderModal.show();
    };
    var showModalWithDelay = function showModalWithDelay(delay) {
      setTimeout(function () {
    ...
    document.getElementById('hearXUnderModal').addEventListener('hidden.bs.modal', function () {
      if (!getCookie(formSubmittedKey)) {
        showModalWithDelay(600000); // Показать снова через 10 минут = 600000
      }
    });

    document.getElementById('hearXUnderForm').addEventListener('submit', function (event) {
      event.preventDefault();
      setCookie(formSubmittedKey, 'true', 365); // Сохранить cookie на 1 год
      var hearXUnderModal = bootstrap.Modal.getInstance(document.getElementById('hearXUnderModal'));
      hearXUnderModal.hide();
    });
  }
  var alertPromotions = document.getElementById('alertPromotions');
  var alertCookie = getCookie('alert-promotions-2023');
  if (alertCookie && alertPromotions) {
    ...
    alertPromotions.addEventListener('closed.bs.alert', function () {
      _setCookie('alert-promotions-2023', 'true', 1);
    });
  }
  ...
  //NO SCROLL
  var bagDropdownMenu = document.querySelector('.bag-dropdown-menu');
  if (bagDropdownMenu) {
    var updateOverflowHidden = function updateOverflowHidden() {
    ...
    var observeBagDropdown = new MutationObserver(function (mutations) {
    ...
    observeBagDropdown.observe(bagDropdownMenu, {
      attributes: true
    });
  }
  ...
  var headerDropdownBag = document.querySelector('.bag-dropdown-menu');
  if (wrapper) {
    var updateWrapperMarginTop = function updateWrapperMarginTop() {
      var navbarHeight = navbar.clientHeight;
      var bagDropdownHeight = 100 * window.innerHeight / 100 - (navbarHeight + 64);
      wrapper.style.marginTop = navbarHeight + 'px';
      dropdownOverflow.style.maxHeight = "".concat(bagDropdownHeight, "px");
    };
    var dropdownOverflow = headerDropdownBag.querySelector('.dropdown-overflow');
  ...
  // HEADER POPOVER
  var popoverBtn = document.getElementById("myPopoverBtn");
  ...
  if (popoverBtn && popover) {
    ...
    document.addEventListener("click", function (event) {
      ...
      var isClickInsidePopoverBtn = popoverBtn.contains(event.target);
      if (!isClickInsidePopover && !isClickInsidePopoverBtn) {
        popover.classList.add("d-none");
      }
    });
  }
  var alertSwiper = new core('.alert-swiper', {
    autoplay: {
      delay: 5000,
  ...
  var heroSwiper = new core('.hero-swiper', {
    autoplay: {
      delay: 5000,
      disableOnInteraction: false
    },
  ...
  var featuredSwiper = new core('.featured-swiper', {
    slidesPerView: 3,
    slidesPerGroup: 1,
    spaceBetween: 50,
    breakpoints: {
  ...
  var heroSwiperImg = document.querySelector('.hero-swiper-img');
  if (heroSwiperImg) {
    var setHeroSwiperPaginationTop = function setHeroSwiperPaginationTop() {
    ...
    var heroSwiperPagination = document.querySelector('.hero-swiper-pagination');
    var heroSwiperImgHeight, heroSwiperPaginationTop;
  ...
  var bannerSwiper = new core('.banner-swiper', {
    spaceBetween: 16,
    autoplay: {
      delay: 5000,
      disableOnInteraction: false
  ...
  var advantagesSwiper = new core('.advantages-swiper', {
    slidesPerView: 3,
    slidesPerGroup: 1,
    spaceBetween: 50,
    breakpoints: {
  ...
  if (document.querySelectorAll(".clock")) {
    var initElements = function initElements(type, prefix) {
      var els = [{}, {}];
      if (!['s', 'm', 'h', 'd'].includes(type)) return els;
      var target = document.querySelector(".".concat(prefix, "flip-clock-").concat(type));
      if (!target) return els;
      var el;
      el = els[0];
      el.digit = target.querySelector(".".concat(prefix, "digit-left"));
      if (el.digit) {
        el.flip = el.digit.querySelector(".".concat(prefix, "flip"));
        el.flipFaces = el.flip.querySelectorAll(".".concat(prefix, "flip-face"));
        el.flipFaceA = el.flipFaces[0];
        el.flipFaceB = el.flipFaces[1];
      }
      el = els[1];
      el.digit = target.querySelector(".".concat(prefix, "digit-right"));
      if (el.digit) {
        el.flip = el.digit.querySelector(".".concat(prefix, "flip"));
        el.flipFaces = el.flip.querySelectorAll(".".concat(prefix, "flip-face"));
        el.flipFaceA = el.flipFaces[0];
    ...
    var runClock = function runClock(prefix, endTime, timeoutId) {
      ...
      now.d = now.d < 10 ? "0".concat(now.d) : "".concat(now.d);
      now.h = now.h < 10 ? "0".concat(now.h) : "".concat(now.h);
      now.m = now.m < 10 ? "0".concat(now.m) : "".concat(now.m);
      now.s = now.s < 10 ? "0".concat(now.s) : "".concat(now.s);
      now.d0 = now.d[1];
      now.d1 = now.d[0];
      now.h0 = now.h[1];
      now.h1 = now.h[0];
      ...
      if (distanceTime == 0) {
        ...
        els.m.forEach(function (minutes) {
        ...
        els.s.forEach(function (seconds) {
        ...
        var index = timeoutIds.indexOf(timeoutId);
        if (index !== -1) {
      ...
      for (var _i = 0, _Object$keys = Object.keys(els); _i < _Object$keys.length; _i++) {
        var t = _Object$keys[_i];
        var _loop = function _loop() {
          var i = _arr[_i2];
          var curr = now["".concat(t).concat(i)];
          var next = curr - 1;
          if (t === 'h' && i === '0' && next < 0) {
            next = 5;
          ...
          if (el && el.digit) {
            if (!el.digit.dataset.digitBefore) {
              ...
              el.flip.addEventListener('transitionend', function () {
                el.digit.dataset.digitBefore = curr;
                el.flipFaceA.textContent = el.digit.dataset.digitBefore;
                var flipClone = el.flip.cloneNode(true);
      ...
      setTimeout(function () {
        return runClock(prefix, endTime);
      }, 1000);
    };
    var timeoutIds = [];
  ...
  var browserBtn = document.querySelector('.btn-browser');
  ...
  //PRODUCT SWIPER
  var productThumbsSlider = new core('.product-slider-thumbs', {
    spaceBetween: 10,
    slidesPerView: 2,
    slidesPerGroup: 1,
  ...
  var productSlider = new core('.product-slider', {
    slidesPerView: 1,
    slidesPerGroup: 1,
    spaceBetween: 10,
    thumbs: {
  ...
  //PRODUCT QUANTITY
  var buttonDecrement = document.querySelectorAll('.button-decrement');
  var buttonIncrement = document.querySelectorAll('.button-increment');
  ...
  if (buttonDecrement && buttonIncrement && productQuantity) {
    var _loop2 = function _loop2(i) {
      buttonDecrement[i].addEventListener('click', function () {
        var currentValue = parseInt(productQuantity[i].value);
        if (currentValue > 1) {
    ...
    var _loop3 = function _loop3(_i3) {
      buttonIncrement[_i3].addEventListener('click', function () {
        var currentValue = parseInt(productQuantity[_i3].value);
        productQuantity[_i3].value = currentValue + 1;
      });
    };
    for (var _i3 = 0; _i3 < buttonIncrement.length; _i3++) {
      _loop3(_i3);
    }
  }

  //PRODUCT REVIEW QUESTIONS
  var userReviewQuestionExists = document.querySelector('.user-review-question');
  var productReviewExists = document.querySelector('.product-review');
  if (userReviewQuestionExists && productReviewExists) {
    document.body.addEventListener('click', function (event) {
      if ((event.target.closest('.user-review-question') || event.target.closest('.user-review-question-duplicate')) && event.target.matches('button')) {
        var userReview = event.target.closest('.product-review');
        var userReviewText = userReview.querySelector('.user-review');
        var userQuestion = userReview.querySelector('.user-review-question');
        var userQuestionDuplicate = userReview.querySelector('.user-review-question-duplicate');
        userQuestion.remove();
  ...
  //RECENT SWIPER
  var recentSwiper = new core('.recent-swiper', {
    slidesPerView: 5,
    slidesPerGroup: 1,
    spaceBetween: 50,
    breakpoints: {
  ...
  //ZOOM PRODUCT ITEM
  var modalZoomSwiper = new core('.modal-zoom-swiper', {
    slidesPerView: 1,
    slidesPerGroup: 1,
    spaceBetween: 50,
    navigation: {
  ...
  var qViewSwiper = new core('.modal-qview-slider', {
    slidesPerView: 1,
    slidesPerGroup: 1,
    spaceBetween: 15,
    navigation: {
    ...
    thumbs: {
      swiper: qViewThumbsSlider,
    },
  });
  var qViewThumbsSlider = new core('.modal-qview-slider-thumbs', {
    spaceBetween: 30,
    slidesPerView: 1,
    slidesPerGroup: 1,
    slideToClickedSlide: true
  ...
  if (document.querySelector('.review')) {
    var ratingForm = document.querySelector('.review-form');
    var cardStars = document.querySelector('.card-stars');
    var iconStars = cardStars.querySelectorAll('.icon-star');
    var ratingInput = document.getElementById('rating-input');
    cardStars.addEventListener('click', function (event) {
      var target = event.target.closest('.icon-star');
      if (!target) return;
      var index = _toConsumableArray(iconStars).indexOf(target);
      iconStars.forEach(function (iconStar, i) {
        if (i <= index) {
    ...
    ratingForm.addEventListener('submit', function (event) {
      if (!ratingInput.value) {
        ...
        window.location.href = "review-thx.html?rating=".concat(ratingValue);
      }*/
    });
  }
  if (document.querySelector('.review-thx')) {
    var urlParams = new URLSearchParams(window.location.search);
    var ratingValue = urlParams.get('rating');
    if (ratingValue) {
      var _cardStars = document.querySelector('.card-stars');
      var _iconStars = _cardStars.querySelectorAll('.icon-star');
      _iconStars.forEach(function (iconStar, i) {
        if (i < ratingValue) {
          iconStar.classList.add('active');
        } else {
          iconStar.classList.remove('active');
  ...
  var returnFormAdd = document.getElementById('addMoreItem');
  var returnForm = document.getElementById('returnForm');
  if (returnForm && returnFormAdd) {
    returnFormAdd.addEventListener('click', function () {
      var newInputs = [{
        type: 'number',
        placeholder: 'Order number *',
        ariaLabel: 'Enter your Order number',
        required: true
      }, {
        type: 'text',
        placeholder: 'Item SKU *',
        ariaLabel: 'Enter your Item SKU',
        required: true
      }];
      newInputs.forEach(function (_ref) {
        var type = _ref.type,
  ...
  if (document.querySelector('.progress-bar-order')) {
    var updateProgressSteps = function updateProgressSteps() {
      var orderProgressBar = document.querySelector('.progress-bar-order');
      var progressValue = parseInt(orderProgressBar.style.width);
      var stepsOrderProgress = [{
        element: document.querySelector('.progress-step-1'),
        threshold: 10
      }, {
        element: document.querySelector('.progress-step-2'),
        threshold: 33
      }, {
        element: document.querySelector('.progress-step-3'),
        threshold: 50
      }, {
        element: document.querySelector('.progress-step-4'),
        threshold: 75
      }, {
        element: document.querySelector('.progress-step-5'),
        threshold: 100
      }];
      stepsOrderProgress.forEach(function (step) {
    ...
    observerProgressBar.observe(orderProgressBar, {
      attributes: true
    });
    updateProgressSteps();
  }
  ...
  //GIFT SWIPER
  var giftCardThumbSwiper = new core(".gift-card-slider-thumbs", {
    spaceBetween: 5,
    slidesPerView: 4,
    freeMode: true,
    watchSlidesProgress: true
  });
  var giftCardSwiper = new core(".gift-card-slider", {
    spaceBetween: 10,
    navigation: {
  ...
  //ACCOUNT SWIPER
  var accountItemSwiper = new core(".account-info-swiper", {
    slidesPerView: 3,
    spaceBetween: 40,
    breakpoints: {
      280: {
  ...
  //AFFILIATE SWIPER
  var affiliateGuideModal = document.querySelector('#affiliateGuideModal');
  if (affiliateGuideModal) {
    affiliateGuideModal.addEventListener('shown.bs.modal', function () {
      var affiliateGuideSwiper = new core(".affiliate-guide-swiper", {
        slidesPerView: 4,
        spaceBetween: 40,
        breakpoints: {
          280: {
    ...
    affiliateGuideModal.addEventListener('hidden.bs.modal', function () {
      affiliateGuideSwiper.destroy();
    });
  }

  //BTN COPY PROMOCODE
  var rewardsCode = document.getElementById('rewardsCode');
  var rewardsCodeBtn = document.getElementById('rewardsCodeBtn');
  if (rewardsCode && rewardsCodeBtn) {
    document.getElementById('rewardsCodeBtn').addEventListener('click', /*#__PURE__*/_asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee() {
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            _context.next = 3;
            return navigator.clipboard.writeText(rewardsCode.textContent);
          case 3:
          ...
          case 6:
            ...
            console.error('Не удалось скопировать промокод:', _context.t0);
          case 9:
          case "end":
  ...
  var qviewHeaders = document.querySelectorAll('.qview-header');
  var qviewHeaderDuplicates = document.querySelectorAll('.qview-header-duplicate');
  var qviewPayProtects = document.querySelectorAll('.pay-protection');
  var qviewPayProtectDuplicates = document.querySelectorAll('.pay-protection-duplicate');
  if (qviewHeaders.length && qviewHeaderDuplicates.length) {
    qviewHeaders.forEach(function (qviewHeader, index) {
  ...
  if (qviewPayProtects.length && qviewPayProtectDuplicates.length) {
    qviewPayProtects.forEach(function (qviewPayProtect, index) {
  ...
  //Page Bag
  var infoProducts = document.querySelectorAll('.product-info');
  var infoProductDuplicates = document.querySelectorAll('.product-info-duplicate');
  if (infoProducts.length && infoProductDuplicates.length) {
    infoProducts.forEach(function (infoProduct, index) {
  ...
  var productHeaderDuplicate = document.querySelector('.product-header-duplicate');
  if (productHeader && productHeaderDuplicate) {
    productHeaderDuplicate.insertAdjacentHTML('beforeend', productHeader.innerHTML);
  }
  var productSupport = document.querySelector('.product-support');
  var productSupportDuplicate = document.querySelector('.product-support-duplicate');
  if (productSupport && productSupportDuplicate) {
    productSupportDuplicate.insertAdjacentHTML('beforeend', productSupport.innerHTML);
  }

  //Page Favorite
  var favoriteBlockBtns = document.querySelectorAll('.block-btn');
  var favoriteBlockBtnDuplicates = document.querySelectorAll('.block-btn-duplicate');
  if (favoriteBlockBtns.length && favoriteBlockBtnDuplicates.length) {
    favoriteBlockBtns.forEach(function (favoriteBlockBtn, index) {
  ...
  var categoryTopDuplicate = document.querySelector('.category-top-duplicate');
  var categoryBottom = document.querySelector('.category-bottom');
  var categoryBottomDuplicate = document.querySelector('.category-bottom-duplicate');
  ...
  //Popup подсказка Affiliate Page
  ...
  if (showMoreButton) {
    showMoreButton.addEventListener('click', function () {
      var hiddenCards = document.querySelectorAll('.category-card-none.d-none');
