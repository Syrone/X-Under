...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var bagCtas = document.querySelectorAll('.bag-cta');
  var orderButtons = document.querySelectorAll('.btn-bag-order');
  ...
  // HEADER BAG
  var closeButtons = document.querySelectorAll('.btn-close-card');
  var dropdownCard = document.querySelector('.dropdown-cards');
  var currentCard = document.querySelector('.current-bag');
  var totalPriceTag = document.querySelector('.total-price');
  var emptyCartMsg = document.querySelector('.empty-cart-msg');
  var bagText = document.querySelector('.bag-text');
  var totalDropdown = document.querySelector('.total-dropdown');
  var btnCheckout = document.querySelector('.btn-checkout');
  var brackets = document.querySelector('.brackets');
  var observer = new MutationObserver(updateCardInfo);
  if (dropdownCard && dropdownCard instanceof Node) {
    observer.observe(dropdownCard, {
      childList: true,
      subtree: true
    });
  }
  ...
  function updateCardInfo() {
    var cards = dropdownCard.querySelectorAll('.dropdown-card');
    var cardCount = cards.length;
    document.querySelectorAll('.current-bag').forEach(function (card) {
      if (cardCount > 0) {
    ...
    if (cards.length === 0) {
      ...
      totalPriceTag.textContent = Array.from(cards).reduce(function (sum, card) {
        return sum + parseFloat(card.querySelector('.card-price').textContent);
      }, 0).toFixed(2);
    }
  }
  ...
  var affiliatePaymentForm = document.querySelector('#paymentMethodForm');
  var paymentSection = document.querySelector('.account-affiliate-payment');
  var affiliateSection = document.querySelector('.account-affiliate');
  if (affiliatePaymentForm) {
    affiliatePaymentForm.addEventListener('submit', function (event) {
      event.preventDefault(); // отменяем стандартное поведение формы

      var formData = new FormData(affiliatePaymentForm); // получаем данные формы
  ...
  if (document.querySelector('.favorite')) {
    ...
    var removeFavoriteItem = function removeFavoriteItem(event) {
      var favoriteItem = event.target.closest('.product-item');
      favoriteItem.classList.add('d-none');

      // Проверяем, есть ли еще элементы в списке избранного
      var favoriteItems = favorite.querySelectorAll('.product-item:not(.d-none)');
    ...
    var favoriteEmpty = document.querySelector('.favorite--empty');
    var btnRemoveList = favorite.querySelectorAll('.btn-remove');
